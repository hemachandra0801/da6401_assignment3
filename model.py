...
class Beam:
    ...
    def update(self, log_probs, hidden):
        """log_probs shape: [current_hypotheses, vocab_size]"""
        new_hyps = []
...
class Transliterator(nn.Module):
    def __init__(self, config):
        ...
        # Encoder
        self.encoder_embed = nn.Embedding(config.dev_vocab_size, config.embed_dim)
        ...
        # Decoder
        self.decoder_embed = nn.Embedding(config.lat_vocab_size, config.embed_dim)
        ...
        self.fc = nn.Linear(config.hidden_dim, config.lat_vocab_size)

    def _adapt_hidden(self, hidden):
        """Adapt encoder hidden state to match decoder layers"""
        if isinstance(hidden, tuple):  # LSTM
            h, c = hidden
            h = h[-self.config.dec_layers :]  # Take last decoder_layers
    ...
    def decode_step(self, lat_input, hidden):
        embedded = self.decoder_embed(lat_input)
        output, hidden = self.decoder_rnn(embedded, hidden)
        logits = self.fc(output.squeeze(1))
        return logits, hidden

    def forward(self, dev_input, target_seq=None, beam_size=1):
        # Encoder
        embedded = self.encoder_embed(dev_input)
        _, hidden = self.encoder_rnn(embedded)
        ...
        if target_seq is not None:
            ...
            return self._beam_search(hidden, dev_input.size(0), beam_size)


    def _teacher_force(self, hidden, target_seq):
        batch_size = target_seq.size(0)
        seq_len = target_seq.size(1)  # Remove the -1 here
        
        outputs = torch.zeros(
            batch_size,
            seq_len,
            self.config.lat_vocab_size
        ...
        for step in range(50):
            ...
            for i in range(batch_size):
                ...
                if isinstance(beam_hidden, tuple):
                    ...
                    new_hidden.append((beam_hidden,))  # Wrap in tuple for consistency
